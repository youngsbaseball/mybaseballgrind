...
function renderResponseBlock(item) {
  ...
  const chipsHtml = chips.length
    ? `<div style="margin-top:8px;line-height:1.7;">
         ${chips.map(c => `<span style="display:inline-block;background:${C.bg};border:1px solid ${C.border};color:${C.gold};padding:4px 10px;border-radius:999px;font-size:13px;margin:2px 4px 2px 0;">${escapeHtml(c)}</span>`).join('')}
       </div>`
    : '';
  ...
  if (totalItems === 0) {
    ...
    for (const [name, arr] of groups.entries()) {
      ...
      for (const it of arr) {
        ...
        if (Array.isArray(r.chips) && r.chips.length) textLines.push(`  ${r.chips.join(' · ')}`);
